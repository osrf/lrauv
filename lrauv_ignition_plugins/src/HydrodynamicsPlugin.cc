...
class HydrodynamicsPrivateData
{
  ...
  public: ignition::math::Vector3d currentVector {0, 0, 0};
  ...
  public: void UpdateCurrent(const ignition::msgs::Vector3d &_msg);
...
/////////////////////////////////////////////////
void HydrodynamicsPrivateData::UpdateCurrent(
...
)
{
      ...
      for(auto k = 0; k < 6; k++)
      {
        ...
        this->dataPtr->stabilityQuadraticAbsDerivative[i*36 + j*6 + k] =
          SdfParamDouble(
            ...
            0);
        ignerr << prefix + "abs" + snameConventionVel[k] <<"\n";
      }
    }
  }
  ...
  {
    this->dataPtr->currentVector = _sdf->Get<ignition::math::Vector3d>("default_current");
  }
      ...
      ignition::gazebo::EntityComponentManager &_ecm)
{
  ...
  // Get current vector
  ignition::math::Vector3d currentVector;
  {
  ...
  // rotation. Also this is where we apply the effects of current to the 
